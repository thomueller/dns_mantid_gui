...
from mantidqtinterfaces.DNSReduction.helpers.converters import el_twotheta_to_d, el_twotheta_to_q
...
class DNSElasticPowderPlotModel(DNSObsModel):
    ...
    def get_max_int_of_workspaces(workspaces):
        ...
        if workspaces:
            return max([
                max(mtd['mat_{}'.format(ws)].extractY()[0])
    ...
    def _datalist_updated(self, workspaces, datalist, scriptnumer):
        compare = ['mat_{}'.format(x) for x in datalist]
...
def _get_y(ws):
    return mtd['mat_{}'.format(ws)].extractY()[0]


def _get_yerr(ws):
    return mtd['mat_{}'.format(ws)].extractE()[0]
