...
class DNSFileSelector_view(DNSView):
    ...
    def __init__(self, parent):
        ...
        self.name = 'Data'
        self._content = load_ui(__file__, 'file_selector.ui', baseinstance=self)
        ...
        self._content.pB_td_read_filtered.clicked.connect(self.read_all_clicked)
        self._content.cB_filter_det_rot.stateChanged.connect(self.filter_scans_checked)
        self._content.cB_filter_sample_rot.stateChanged.connect(self.filter_scans_checked)
        self._content.cB_filter_scans.stateChanged.connect(self.filter_scans_checked)
        self._content.cB_filter_cscans.stateChanged.connect(self.filter_scans_checked)
        self._content.cB_filter_free.stateChanged.connect(self.filter_scans_checked)
        self._content.lE_filter_free_text.textChanged.connect(self.filter_scans_checked)
        ...
        self._content.pB_check_last_scan.clicked.connect(self.check_buttons_clicked)
        self._content.pB_check_last_complete_scan.clicked.connect(self.check_buttons_clicked)
        self._content.pB_check_selected.clicked.connect(self.check_buttons_clicked)
        self._content.groupBox_filter_standard.setHidden(1)
        self.standard_treeview.setHidden(1)
        self._content.cB_filter_vanadium.stateChanged.connect(self.filter_standard_checked)
        self._content.cB_filter_nicr.stateChanged.connect(self.filter_standard_checked)
        self._content.cB_filter_empty.stateChanged.connect(self.filter_standard_checked)
        # this defines also which elements are saved, there is a creator which creates the mapping
        ## we do not atomatically make the names for readability
        #mapper = mapping_creator(self._content)
        self._mapping = {'filter_scans': self._content.cB_filter_scans,
        ...
        self._content.combB_directory.currentIndexChanged.connect(self.combo_changed)
    ...
    def get_filters(self):
        """
        returns chosen filters which should be applied to the list of scans
        """
        state_dict = self.get_state()
        freetext = state_dict['filter_free_text']
        filters = {'det_rot' : state_dict['filter_det_rot'],
                   'sample_rot' : state_dict['filter_sample_rot'],
                   ' scan' : state_dict['filter_scans'], ## space is important to not get cscans
    ...
    def get_standard_filters(self):
        state_dict = self.get_state()
        filters = {'vanadium' : state_dict['filter_vanadium'],
    ...
    def open_progress_dialog(self, numofsteps):
        self.progress = QProgressDialog("Loading {} files...".format(numofsteps), "Abort Loading", 0, numofsteps)
