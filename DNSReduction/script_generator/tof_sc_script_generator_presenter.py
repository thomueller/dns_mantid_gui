...
class DNSTofScScriptGenerator_presenter(DNSScriptGenerator_presenter):
    ...
    def __init__(self, parent):
        super(DNSTofScScriptGenerator_presenter, self).__init__(parent, 'tof_sc_script_generator')
    ...
    def script_maker(self):
        ...
        extents = ','.join([str(tof_opt[x]) for x in ['hmin', 'hmax', 'kmin', 'kmax', 'lmin', 'lmax', 'dEmin', 'dEmax']])
        bins = ','.join([str(tof_opt[x]) for x in ['hbins', 'kbins', 'lbins', 'dEbins']])
        l("from __future__ import (absolute_import, division, print_function, unicode_literals)")
        ...
        l("bins = '{}'".format(bins))
        l()
        l("raw_data = BinMD('raw_edata', AxisAligned='0', BasisVector0=bvector0, BasisVector1=bvector1, BasisVector2=bvector2,"
          + "\n                 BasisVector3=bvector3, OutputExtents=extents, OutputBins=bins, NormalizeBasisVectors='0')")
        l("data_norm = BinMD('edata_norm', AxisAligned='0', BasisVector0=bvector0,  BasisVector1=bvector1, BasisVector2=bvector2,"
          + "\n                 BasisVector3=bvector3, OutputExtents=extents, OutputBins=bins, NormalizeBasisVectors='0')")
