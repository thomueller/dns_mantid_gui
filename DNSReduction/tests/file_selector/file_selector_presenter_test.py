...
from mantidqtinterfaces.DNSReduction.file_selector.file_selector_presenter import \
    DNSFileSelectorPresenter
from mantidqtinterfaces.DNSReduction.file_selector.file_selector_watcher import \
    DNSFileSelectorWatcher

from mantidqtinterfaces.DNSReduction.file_selector.file_selector_view import DNSFileSelectorView
from mantidqtinterfaces.DNSReduction.tests.helpers_for_testing import get_fileselector_param_dict
...
class DNSFileSelectorPresenterTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.view.sig_autoload_clicked = mock.Mock()
    ...
    @patch('DNSReduction.file_selector.file_selector_presenter.'
    ...
    def test_read_all(self, mock_filter, mock_startend):
        ...
        self.model.read_all.assert_called_once_with(
            ['a'], 'C:/data', ['b'], False)
    ...
    def test_read_filtered(self, mock_read_all):
        self.view.get_start_end_filenumbers.return_value = [1, 2]
        self.presenter._read_filtered()
        self.view.get_start_end_filenumbers.assert_called_once()
    ...
    def test_set_start_end(self):
        ...
        testf = self.view.set_start_end_filenumbers_from_arguments
        testf.assert_called_once_with(0, 1)

    @patch('DNSReduction.file_selector.file_selector_presenter.'
           'DNSFileSelectorPresenter._read_all')
    def test__autoload(self, mock_read_all):
        self.presenter.param_dict = get_fileselector_param_dict()
        self.presenter._old_data_set = [1]
        self.presenter._autoload(1)
    ...
    @patch('DNSReduction.file_selector.file_selector_presenter.'
           'DNSFileSelectorPresenter._check_all_vissible_scans')
    @patch('DNSReduction.file_selector.file_selector_presenter.'
           'DNSFileSelectorPresenter._read_standard')
    def test_automatic_select_all_standard_files(
        self, mock_read_standard, mock_check):
    ...
    def test_check_all_vissible_scans(self, mock_hiddenrow):
        mock_hiddenrow.return_value = [1, 2]
        self.presenter._check_all_vissible_scans()
    ...
    def test_get_option_dict(self):
        ...
        self.model.get_data.assert_any_call(fullinfo=False)
    ...
    def test_process_request(self, mock_get_option, mock_auto):
        ...
        mock_get_option.return_value = {'auto_standard': True,
                                        'standard_data': []}
        self.presenter.process_request()
    ...
    def test_set_view_from_param(self, mock_print):
        self.presenter.param_dict = get_fileselector_param_dict()
        self.model.check_by_filenumbers.return_value = []
        self.presenter.set_view_from_param()
        self.view.set_state.assert_called_once_with({})
        self.model.check_by_filenumbers.assert_called_once_with(
            [796640, 796640])
        self.model.check_by_filenumbers.return_value = [1, 2]
        self.presenter.set_view_from_param()
        mock_print.assert_called_once_with('Of 0 loaded checked filenumbers'
                                           ' [1, 2] were not found '
