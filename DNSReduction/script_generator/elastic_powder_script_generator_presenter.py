...
class DNSElasticPowderScriptGenerator_presenter(DNSScriptGenerator_presenter):
    ...
    def __init__(self, parent):
        super(DNSElasticPowderScriptGenerator_presenter, self).__init__(parent, 'elastic_powder_script_generator')

    def create_dataset(self, data):
        dataset = {}
        for entry in  data:
            if entry['samplename'] == 'leer': ## compatibility with old dnsplot names for empty
                datatype = 'empty'
            else:
        ...
        for sample_typ, fields in dataset.items():
            for field, filenumbers in fields.items():
                if field != 'path':
                    dataset[sample_typ][field] = list_to_multirange(filenumbers)
        return dataset
    ...
    def format_dataset(self, dataset):
        ...
        for samplename, fields in dataset.items():
            if not dataset_string.endswith('{'):
                ...
                dataset_string += "\n "
            dataset_string += " "*(longest_samplename-len(samplename)) + "'{:s}' : {{".format(samplename)
            spacing = len(" "*(longest_samplename-len(samplename)) + "'{:s}' : {{".format(samplename))+1
            dataset_string += "'path'  : '{}'".format(fields['path'])
            for field, filenumbers in sorted(fields.items()):
                if field != 'path':
                    dataset_string += ",\n" + " "*spacing + "'{:s}'".format(field) + " "*(5-len(field))
    ...
    def script_maker(self):
        self.script = [""]
        options = self.param_dict['options']
        def l(line=""):
            self.script += [line]
        sample_data = self.param_dict['file_selector']['full_data']

        det_rot = [entry['det_rot'] for entry in sample_data]
        diff_rot = [abs(det_rot[i] - det_rot[i+1])  for i in range(len(det_rot) - 1)]
        diff_rot = [i for i in diff_rot if i >= 0.03]
        avg_diff = np.average(diff_rot)
        if avg_diff <= 0.1 or not diff_rot:
            avg_diff = 1
        stepfactor = round(5 /avg_diff , 0)/5 # stepsize correction for dns uncertainty
        twothetastep = 1/stepfactor
        twotheta_max = -min(det_rot) + 115
        twotheta_min = -max(det_rot)
        numberofbins = int(round((twotheta_max - twotheta_min) * stepfactor + 1, 0))
        twotheta_min = twotheta_min - twothetastep / 2.0 ### to have bin center around measurement point
        ...
        importstring = 'from DNSReduction.scripts.md_powder_elastic import load_all, background_substraction'
        if options['corrections'] and (options['det_efficency'] or options['flipping_ratio']):
        ...
        l("binning = ['twoTheta' , {:.2f}, {:.2f}, {:d}] # min, max, number_of_bins".format(twotheta_min, twotheta_max, numberofbins))
        l("wss_sample = load_all(sample_data, binning, {})".format(normalizestring))
        l("wss_standard = load_all(standard_data, binning, {}, standards=True)".format(normalizestring))
        l()
        ...
        if options['background_factor'] != 1:
            factorstring = ", factor={}".format(options['background_factor'])
        else:
            factorstring = ""
        if len(sample_data.keys()) == 1:
            samplename = sample_data.keys()[0]
            spacing = 4
            correctionstring = "for workspace in wss_sample['{}']:".format(samplename)
        else:
            spacing = 8
            correctionstring = ("for sample, workspacelist in wss_sample.items(): " +
                                "\n    for workspace in workspacelist:")
        correctionstring += "\n" + spacing*" " + "background_substraction(workspace{})".format(factorstring)
        if options['corrections'] and options['det_efficency']:
            if options['det_efficency']:
                correctionstring += "\n" + spacing*" " + "vanadium_correction(workspace, vanaset=standard_data['vana']"
        ...
        if options['corrections'] and options['flipping_ratio']:
            if len(sample_data.keys()) == 1:
                ...
                correctionstring = ("for sample, workspacelist in wss_sample.items(): " +
                                    "\n    for workspace in workspacelist:")
            correctionstring += "\n" + spacing*" " + "fliping_ratio_correction(workspace)"
        l(correctionstring)
        if len(sample_data.keys()) == 1:
            samplename = sample_data.keys()[0]
            spacing = 4
            ...
            correctionstring = ("for sample, workspacelist in wss_sample.items(): " +
                                "\n    for workspace in workspacelist:")
        correctionstring += "\n" + spacing*" " + "ConvertMDHistoToMatrixWorkspace(workspace, Outputworkspace='mat' + workspace, Normalization='NoNormalization')"  
        l(correctionstring)
        if options["separation"] and options['separation_xyz']:
            l()
            l("## do xyz polarization analysis for magnetic powders")
            for sample, fields in sample_data.items():
                if not all(field in fields for field in ['x_sf', 'y_sf', 'z_sf', 'z_nsf']):
                    self.raise_error('Not all fields necesarry found for XYZ analysis.')
                else:
                    l("{0}_nuclear_coh, {0}_magnetic, {0}_spin_incoh = xyz_seperation(x_sf='{0}_x_sf',".format(sample) +
                     "\n" + " "*(len(sample)+64) +"y_sf='{}_y_sf',".format(sample) +
                     "\n" + " "*(len(sample)+64) +"z_sf='{}_z_sf',".format(sample) +
                     "\n" + " "*(len(sample)+64) +"z_nsf='{}_z_nsf')".format(sample))
                    l("ConvertMDHistoToMatrixWorkspace('{0}_nuclear_coh', Outputworkspace='mat_{0}_nuclear_coh', Normalization='NoNormalization')".format(sample))
                    l("ConvertMDHistoToMatrixWorkspace('{0}_magnetic', Outputworkspace='mat_{0}_magnetic', Normalization='NoNormalization')".format(sample))
                    l("ConvertMDHistoToMatrixWorkspace('{0}_spin_incoh', Outputworkspace='mat_{0}_spin_incoh', Normalization='NoNormalization')".format(sample))
        if options["separation"] and options['separation_coh_inc']:
            nsf_sf_pairs = []
            l()
            for sample, fields in sample_data.items():
                for field in fields.keys():
                    if field.endswith('_sf') and field[:-2]+'nsf' in sample_data[sample].keys():
                        nsf_sf_pairs.append("{}_{}".format(sample, field[:-3]))
                if not nsf_sf_pairs:
                    self.raise_error('No pair of SF and NSF measurements found for {}'.format(sample))
            l("## sepearation of coherent and incoherent scattering of non magnetic sample")
            for sample_field in sorted(nsf_sf_pairs): # normally only z is measured but just in case
                l("{0}_nuc_coherent, {0}_spin_incoherent = non_mag_sep('{0}_sf', '{0}_nsf')".format(sample_field))
