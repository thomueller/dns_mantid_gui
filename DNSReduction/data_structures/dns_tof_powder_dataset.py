...
from mantidqtinterfaces.DNSReduction.data_structures.dns_dataset import (get_bank_positions,
                                                      get_proposal_from_filname
                                                      )
from mantidqtinterfaces.DNSReduction.data_structures.object_dict import ObjectDict
from mantidqtinterfaces.DNSReduction.helpers.list_range_converters import \
...
def _get_field_string(fields, llens):
    ...
    spacer = " " * (llens + 1)
    stringlist = [("{}{:4.2f}: {}").format(spacer, key, value)
...
def _get_sample_string(samplename, llens):
    return "'{:s}': {{".format(samplename).rjust(llens)
    ...
    def create_dataset(data, path):
        ...
        for entry in data:
            ...
            if datatype in dataset.keys():
                ...
                _create_new_datatype(dataset, datatype, det_rot, entry,
                                     path)
