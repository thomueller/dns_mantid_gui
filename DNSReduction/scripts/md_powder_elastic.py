# -*- coding: utf-8 -*-
"""
Created on Mon Aug 12 14:05:03 2019

@author: thomasm
"""
from __future__ import (absolute_import, division, print_function)
import numpy as np
from mantid.simpleapi import LoadDNSSCD, mtd, BinMD, DivideMD, MultiplyMD, MinusMD, CreateSingleValuedWorkspace, IntegrateMDHistoWorkspace, PlusMD, GroupWorkspaces
...
def load_binned(workspacename, binning, path, filenumbers, normalizeto):
    filepaths = []
    ad0 = 'Theta,{},{},{}'.format(binning[1]/2.0, binning[2]/2.0, binning[3])
    two_theta_limits = "{},{}".format(binning[1], binning[2])
    ad1 = 'Omega,0.0,359.0,1'
    ad2 = 'TOF,424.0,2000.0,1'
    binning = [ad0, ad1, ad2]
    filepaths.append(["{}_{}.d_dat".format(path, number) for number in filenumbers])
...
def load_all(data_dict, binning, normalizeto='monitor', standards=False):
    workspacenames = {}
    for samplename, fields in data_dict.items():
        ...
        for fieldname, filenumbers in fields.items():
            if fieldname != 'path':
                ...
                load_binned(workspacename, binning, path, filenumbers, normalizeto)
...
def vanadium_correction(workspacename, vanaset=None, ignore_vana_fields=False, sum_vana_sf_nsf=False):
    workspacenorm = '_'.join((workspacename, 'norm'))
    if workspacename.endswith('_sf'):
        fieldname = workspacename[-4:]
    else:
    ...
    if ignore_vana_fields:
        if vanaset:
            ...
            for field in vanaset:
                if field != 'path':
                    ...
                    try:
                        ...
                        raise_error('No vanadium file for field {}. '.format(fieldname))
                        return mtd[workspacename]
            ...
            raise_error('Need to give vanadium dataset explicit if you want all vandium files to be added.')
        ...
        try:
            ...
            raise_error('No vanadium file for {}_sf . You can choose to ignore vanadium fields in the options.'.format(polarization))
            return mtd[workspacename]
        try:
            vana_nsf = mtd[vana_nsf]
            vana_nsf_norm = mtd[vana_nsf_norm]
        except KeyError:
            raise_error('No vanadium file for {}_nsf. You can choose to ignore vanadium fields in the options.'.format(polarization))
            return mtd[workspacename]
        ...
        try:
            ...
            raise_error('No vanadium file for {}. You can choose to ignore vanadium fields in the options.'.format(fieldname))
            return mtd[workspacename]
...
def fliping_ratio_correction(workspace):
    ...
    try:
        ...
        raise_error('no matching NiCr workspace found for {}'.format(workspace))
        return
    ...
    mtd[sf_workspace] = MinusMD(sf_workspace, diff_ifr, OutputWorkspace=sf_workspace)
    mtd[nsf_workspace] = PlusMD(nsf_workspace, diff_ifr, OutputWorkspace=nsf_workspace)
...
def xyz_seperation(x_sf, y_sf, z_sf, z_nsf):
    ...
    seperated = GroupWorkspaces(['{}_magnetic'.format(samplename), '{}_spin_incoh'.format(samplename), '{}_nuclear_coh'.format(samplename)])
    return [mtd['{}_nuclear_coh'.format(samplename)], mtd['{}_magnetic'.format(samplename)], mtd['{}_spin_incoh'.format(samplename)]]

def non_mag_sep(sf_workspace, nsf_workspace):
    samplename = sf_workspace[:-5]
    mtd['{}_nuclear_coh'.format(samplename)]  = mtd[nsf_workspace] -0.5*  mtd[sf_workspace]
    mtd['{}_spin_incoh'.format(samplename)] = 1.5 * mtd[sf_workspace]
    seperated = GroupWorkspaces(['{}_nuclear_coh'.format(samplename), '{}_spin_incoh'.format(samplename)])
    return [mtd['{}_nuclear_coh'.format(samplename)], mtd['{}_spin_incoh'.format(samplename)]]
