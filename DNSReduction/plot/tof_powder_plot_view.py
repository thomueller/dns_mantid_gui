...
class DNSTofPowderPlot_view(DNSView):
    ...
    def __init__(self, parent):
        ...
        self._content = load_ui(__file__, 'tof_powder_plot.ui', baseinstance=self)
        self.name = 'Plotting'
        ...
        self._mapping = {'plot_button': self._content.pB_plot,
        ...
        self._mapping['plot_button'].clicked.connect(self.plot)
        ...
        for m in colormaps:
            if self.within_mantid:
                self._mapping['colormap'].addItem(QIcon("scripts/DNSReduction/plot/colormaps/"+m+".png"), m)
            else:
                self._mapping['colormap'].addItem(QIcon("DNSReduction/plot/colormaps/"+m+".png"), m)

        self._mapping['colormap'].setCurrentIndex(colormaps.index('viridis'))
        self._mapping['colormap'].currentIndexChanged.connect(self.set_colormap)
        self._mapping['reverse_colormap'].stateChanged.connect(self.set_colormap)
    ...
    def update_min(self, slidervalue):
        self.minimum = (self.plotmax - self.plotmin) * slidervalue / 1000.0 + self.plotmin
        self.cl.set_clim(vmin=self.minimum, vmax=self.maximum)
        self.ax.figure.canvas.draw()
        self.static_canvas.figure.tight_layout()

    def update_max(self, slidervalue):
        self.maximum = (self.plotmax - self.plotmin) * slidervalue / 1000.0 + self.plotmin
        self.cl.set_clim(vmin=self.minimum, vmax=self.maximum)
        self.ax.figure.canvas.draw()
        self.static_canvas.figure.tight_layout()
