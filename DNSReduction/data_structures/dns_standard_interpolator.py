...
from mantidqtinterfaces.DNSReduction.helpers.file_processing import get_path_and_prefix, create_dir

def read_standard_file(path, number):
    path, prefix = get_path_and_prefix(path)
    filename = '{}_{:06d}.d_dat'.format(prefix, number)
...
def write_inp_file(arr, path, filenumb_dict, filenumber, prefix):
    ...
    for line in arr:
        ...
        filename = '{}_{:06d}.d_dat'.format(propnb, number)
        ...
        newfilename = '{}_{:06d}.d_dat'.format(prefix, filenumber)
...
def interpolate_standard(standard_data, bank_positions, scriptname):
    ...
    for sample_type in standard_data.keys():
        ...
        newpath = os.path.dirname(path) + '/interp/'
