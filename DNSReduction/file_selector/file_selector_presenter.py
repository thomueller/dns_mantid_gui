...
def return_filelist(datadir):
    """
    returns list of names of dnsfiles in datadir
    """
    filelist = glob.glob(datadir + "/*.d_dat")
...
def filter_filenames(alldatafiles, start, end):
    """
    filters datafilenames to the range given in sB_td_file_nb and sB_td_file_to
    """
    filtered = []
...
class DNSFileSelector_presenter(DNSObserver):
    ...
    def __init__(self, parent):
        super(DNSFileSelector_presenter, self).__init__(parent, 'file_selector')
    ...
    def dataset_changed(self, standard_set):
        if standard_set:
            ...
            self.active_model = self.standard_data  # some buttons are for boths models
    ...
    def read_standard(self):
        """
        reads new files, if filtered is True, only the files in the range specified
        """
    ...
    def read_all(self, filtered=False, watcher=False):
        ...
        if watcher: #sequential load called
            datafiles = [dfile for dfile in alldatafiles if dfile not in self.old_data_set]#
        ...
        if model.numberOfScans() == 1:
            self.view.expand(alle=True)
        if tofmode:
            self.view.show_statusmessage("{} TOF files loaded".format(total_files))
        else:
            self.view.show_statusmessage("{} elastic files loaded".format(total_files))
    ...
    def filter_scans(self):
        """
        hides and unchecks the scans in the treeview which do not match filters from
        get_filters
        """
        model = self.model
        ...
        for row in model.scanRange():
            ...
            for text, filter_condition in filters.items():
                if filter_condition and not text in model.scanCommandFromRow(row):
    ...
    def filter_standard(self):
        ...
        for row in model.scanRange():
            ...
            for child in scan.get_childs():
                if not filters['vanadium'] and not filters['empty'] and not filters['nicr']:
    ...
    def check_buttons(self, sender_name):
        """
        processes the button clicks to check All None, Last scan, Last
        ...
        if 'last' in sender_name:
            ...
            for i in range(number_of_scans_to_check):
                while (row - i) >= 0 and (((view.is_scan_hidden(row - i)) or
                                           ('scan' not in  model.scanCommandFromRow(row - i)))):
                    row += -1  # do not check hidden rows
                while  (row - i) >= 0 and (('scan' not in model.scanCommandFromRow(row - i)) or
                                           (view.is_scan_hidden(row - i)) or
                                           (('complete' in sender_name) and
                                            (row - i > 0) and
    ...
    def set_view_from_param(self):
        """
        sets the fields defined in mapping from the parameter dictionary
        and checks scans checked in the dict
        """
        filenumbers = self.param_dict.get(self.name, {}).get('selected_filenumbers', [])
        ...
        for filenb in filenumbers:
            try:
                ...
                notfound += 1
        if notfound:
            print('Of {} loaded checked filenumbers {} were not found in list of datafiles'.format(len(filenumbers), notfound))

    def get_option_dict(self):
        if self.view is not None:
            self.own_dict.update(self.view.get_state())
        self.own_dict['full_data'] = self.model.get_checked(fullinfo=True)
        self.own_dict['standard_data'] = self.standard_data.get_checked(fullinfo=True)
        #print(self.own_dict['full_data'])
        #print(self.own_dict['standard_data'])
    ...
    def autoload(self, state):
        """ turns on an off watcher for changes in data directory """
    ...
    def process_request(self):
        ...
        if own_options['auto_standard'] and not own_options['standard_data']:
            ...
            for row in model.scanRange():
                if not view.is_scan_hidden(row):
                    model.setCheckedScan(row, 2)
            self.view.show_statusmessage('automatically loaded all standard files', 30)
