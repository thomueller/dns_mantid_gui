...
"""
Defines DNS measurement modi and allows switching
"""
...
from DNSReduction.options.options_presenter import DNSOptions_presenter
...
class DNSModus(object):
    ...
    def __init__(self, name, parent):
        ...
        self.parent = parent ##shold be main presenter
        self.name = name ## only names which are keys in the mapping below are allowed
        self.presenters = OrderedDict()

        self.mode_map = {
            'powder_elastic' : ['path', 'file_selector', 'options', 'elastic_powder_script_generator', 'xml_dump', 'plot_elastic_powder'],
            'powder_tof'     : ['path', 'file_selector',
                                'tof_powder_options',
                                'tof_powder_script_generator',
                                'xml_dump', 'plot_tof_powder'],
            'sc_elastic'     : ['path', 'xml_dump'],
            'sc_tof'         : ['path', 'file_selector', 'tof_sc_options', 'tof_sc_script_generator', 'xml_dump'],
            'simulation'     : ['simulation'],
            }
        self.mode_map['all'] = set([j for i in self.mode_map.values() for j in i])

        self.presenter_map = {
            'path'                : DNSPath_presenter(self.parent),
            'file_selector'       : DNSFileSelector_presenter(self.parent),
            'options'             : DNSOptions_presenter(self.parent),
            'tof_powder_options'         : DNSTofPowderOptions_presenter(self.parent),
            'tof_powder_script_generator': DNSTofPowderScriptGenerator_presenter(self.parent),
            'elastic_powder_script_generator': DNSElasticPowderScriptGenerator_presenter(self.parent),
            'xml_dump'            : DNSXMLDump_presenter(self.parent),
            'simulation'          : DNSSimulation_presenter(self.parent),
            'plot_tof_powder'     : DNSTofPowderPlot_presenter(self.parent),
            'tof_sc_options' : DNSTofScOptions_presenter(self.parent),
            'tof_sc_script_generator': DNSTofScScriptGenerator_presenter(self.parent),
            'plot_elastic_powder'     : DNSElasticPowderPlot_presenter(self.parent),
