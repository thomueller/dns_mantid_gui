...
class DNSSimulation_view(DNSView):
    ...
    def __init__(self, parent):
        ...
        self._content.dSB_sample_rot.valueChanged.connect(self.sample_rotchanged)
        ...
        self._content.dSB_wavelength.valueChanged.connect(self.wavelength_changed)
        ...
        self._content.table.setEditTriggers(QtWidgets.QTableWidget.NoEditTriggers)
        self._content.table.sortByColumn(3, 0) # sort after q unlesss user changes it

        self._mapping = {'powder_start': self._content.dSB_powder_start,
        ...
        ### setting up matplotlib widgets - should be moved to designer ui file
        ### need to make custom widgte for that
        ...
        self.sc_static_canvas.mpl_connect('axes_leave_event', self.mouseoutplot)
    ...
    def fixomegaoffset(self, checked):
        ...
        if checked:
            ...
            self._content.l_warning_ooset.setText('Warning: omega offset not set')
        return

    def tableitemdclicked(self, item):
        dr = float(self._content.table.item(item.row(), item.column()-2).text())
        sr = float(item.text())
    ...
    def set_hkl_position_on_plot(self, hkl):
        self._content.l_show_hkl.setText('hkl = [{:5.2f} , {:5.2f} , {:5.2f}]'.format(*hkl))

    def clear_d_tooltip(self):
        """called if hkl changed but no new calculation of d was done"""
        self._mapping['hkl1'].lE_hkl1.setToolTip('')
        self._mapping['hkl2'].lE_hkl2.setToolTip('')
        self._mapping['hkl2_p'].lE_hkl2_p.setToolTip('')
        return
    ...
    def get_state(self):
        ...
        for hkl in ['hkl1', 'hkl2']:
            state_dict[hkl+'_v'] = sim_help.hkl_string_to_float_list(state_dict[hkl])
        return state_dict
    ...
    def open_filename_dialog(self):
        ...
        file_name = QFileDialog.getOpenFileName(self,
                                                ...
                                                options=options)
        if isinstance(file_name, tuple): ## Qt4 vs QT5 hack QT5 returns string with filter as second argument
    ...
    def set_hkl2_p(self, q2_p):
        self.set_single_state(self._mapping['hkl2_p'], "[{0:5.3f},{1:5.3f},{2:5.3f}]".format(*q2_p))
    ...
    ### have to replace this with qtableview and custom qabstractitem model
    def writetable(self, refls, tthlimit):#
        ...
        tablehead = ['h', 'k', 'l', 'q', 'd', 'tth', 'fs', 'M',
                     'diff', 'det_rot', 'channel', 'sample_rot']
        self._content.table.setColumnCount(len(tablehead))

        self._content.table.setRowCount(len(refls))
        formatstring = [' {0:.0f} ', ' {0:.0f} ', ' {0:.0f} ', ' {0:.2f} ',
                        ' {0:.2f} ', ' {0:.2f} ', ' {0:.0f} ', ' {0:.0f} ',
                        ' {0:.2f} ', ' {0:.2f} ', ' {0:.0f} ', ' {0:.2f} ']
        row = 0
        for refl in refls:
            for col, head in enumerate(tablehead):
                cellinfo = MyTableWidgetItem(formatstring[col].format(getattr(refl, head)))
                cellinfo.setTextAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
                if tablehead[col] == 'M':
    ...
    def powderplot(self, x, y, refl_to_annotate):
        ...
        for i, hkl in enumerate(refl_to_annotate[1]):
            ax.annotate('  [{:4.2f}, {:4.2f}, {:4.2f}]'.format(*hkl),
                        (refl_to_annotate[0][i], refl_to_annotate[2][i]), fontsize=10)
    ...
    def sc_plot(self, line, refls, maxI, minI, q1, q2):
        ...
        cm = get_cmap('plasma')
        sc = ax.scatter(refls[:, 0], refls[:, 1], c=refls[:, 2],
                        vmin=minI, vmax=maxI, s=300, cmap=cm, zorder=20)
        for i in range(len(refls[:, 0])):
            ax.annotate('    {:.0f}\n    [{:5.2f}, {:5.2f}, {:5.2f}]'.format(
                refls[i, 2], refls[i, 3], refls[i, 4],
                refls[i, 5]), (refls[i, 0], refls[i, 1]), fontsize=10, zorder=200)

        ax.set_xlabel(q1, fontsize=14)
        ax.set_ylabel("[{0:5.3f}, {1:5.3f}, {2:5.3f}]".format(*q2), fontsize=14)
