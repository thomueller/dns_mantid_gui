...
from mantid.geometry import ReflectionConditionFilter, OrientedLattice, UnitCell
...
class DNSSimulation_model(DNSObsModel):
    ...
    def __init__(self, parent):
        ...
        self.sample_ws = CreateSampleWorkspace(OUTPUTWORKSPACE='simulation')
        self.orilat = None
        self.refls = None
        self.generator = None
        self. cryst = None
        self.non_rot_lat = None
        #self.own_dict is shared with presenter

    def loadCIF(self, fileName):
        """uses mantid to load CIF and set crystalstructure and oriented
        lattice, calls set unitcell"""
        LoadCIF(self.sample_ws, fileName)
        self.cryst = self.sample_ws.sample().getCrystalStructure()
    ...
    def setcellfromparameters(self):
        ...
        if spacegroup.isdigit(): # if user gives SG number convert to HM
            spacegroup = SpaceGroupFactory.subscribedSpaceGroupSymbols(int(spacegroup))
            spacegroup = spacegroup[0] # number is not unique just use one rep.
    ...
    def get_refls_and_set_orientation(self):
        ...
        self.orilat.setUFromVectors(sim_help.listtoV3D(q1), sim_help.listtoV3D(q2))
        self.non_rot_lat.setUFromVectors(sim_help.listtoV3D(q1), sim_help.listtoV3D(q2))
        ...
        hkls_unique = self.generator.getUniqueHKLsUsingFilter(1 / maxq, 100, reffilter) # dived by 2pi
        hkls = self.generator.getHKLsUsingFilter(1 / maxq, 100, reffilter) # dived by 2pi
        ...
        for i in range(len(hkls)-1, 0, -1): #reverse order to get positive hkl
            refl = SimReflStruc()
            refl.hkl = hkls[i]
            ...
            if not self.own_dict['cif_filename']: # if dummy scatterer set I = 1
                refl.fs = 1
                refl.fs_lc = 1
            refl.h = refl.hkl[0]
            refl.k = refl.hkl[1]
            ...
            refl.channel, refl.det_rot = sim_help.shift_channels_below_23(refl.channel, refl.det_rot)
            refl.omega = sim_help.hkl_to_omega(refl.hkl, UB, wavelength, refl.tth)
            refl.sample_rot = sim_help.omega_to_samp_rot(refl.omega, refl.det_rot)
    ...
    def filter_refls(self, refls):
        ...
        if inplane and not unique:
            ...
            for refl in refls:
                if refl.inplane:
                    if not any(refl.hkl in nrefl.equivalents for nrefl in newrefls):
                        newrefls.append(refl)
            refls = newrefls
        return refls

    def get_ds(self):
        [d_hkl1, d_hkl2, d_hkl2_p] = self.generator.getDValues([sim_help.listtoV3D(self.own_dict['hkl1_v']),
    ...
    def create_dns_surface(self, q1, q2):
        ...
        wavelength = self.own_dict['wavelength']
        tth_rang = np.linspace(tth_start, tth_end, int(abs((tth_end - tth_start))))
        omega_rang = np.linspace(omega_start, omega_end, int(abs(omega_end - omega_start)))
        surface = sim_help.return_dns_surface_shape(self.orilat, tth_rang, omega_rang, q1, q2, wavelength)
        return surface
